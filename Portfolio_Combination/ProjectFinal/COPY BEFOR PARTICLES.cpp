...
int trunkList, leavesList; // obj file lists
int TextureSetting;
char PlanetSetting;
...
Display( )
{
	...
	gluLookAt( 6.f, 4.f, 6.f,     0.f, 0.f, 0.f,     0.f, 1.f, 0.f );
	...
	SetPointLight( GL_LIGHT0, 1., 1., 0., 1., 1., 1. );
	...
	//BOOLEAN　to extract trunk and have two obj files
	glCallList( trunkList ); // put a cat in the scene
	glPopMatrix();
	...
	//BOOLEAN　to extract trunk and have two obj files
	glCallList( leavesList ); // put a cat in the scene
	glPopMatrix();
...
InitGraphics( )
{
	...
	// glGenTextures( 1, (GLuint *) &MarsTex );
...
// Now you have 7 texture IDs stored in the textureIDs array, and you can use them to map textures to your objects.
...
InitLists( )
{
	...
	// Source: https://www.turbosquid.com/3d-models/3d-assets-tree-grass-rocks-1498368
	// create nature OBJ
	trunkList = glGenLists( 1 );
	glNewList( trunkList, GL_COMPILE );
		LoadObjFile( (char *) "Tree.obj");
	glEndList( );
	...
	glNewList( SpaceDL, GL_COMPILE );
		...
		glPushMatrix( );
			glScalef( 10.f, 10.f, 10.f );	// scale of venus  sphere, from the table
	...
	// 	glBindTexture( GL_TEXTURE_2D, TextureIDs[1] );	// earth Tex must have already been created when this is called
	// 	glPushMatrix( );
	// 		glCallList( SphereDL );		// a dl can call another dl that has been previously created
	// 	glPopMatrix( );
	// glEndList( );
	...
	// 	glBindTexture( GL_TEXTURE_2D, TextureIDs[2] );	// MarsTex must have already been created when this is called
	// 	glPushMatrix( );
	...
	// 	glBindTexture( GL_TEXTURE_2D, TextureIDs[3] );	// JupiterTex must have already been created when this is called
	// 	glPushMatrix( );
	...
	// 	glBindTexture( GL_TEXTURE_2D, TextureIDs[4] );	// SaturnTex must have already been created when this is called
	// 	glPushMatrix( );
	...
	// 	glBindTexture( GL_TEXTURE_2D, TextureIDs[5] );	// UranusTex must have already been created when this is called
	// 	glPushMatrix( );
	...
	// 	glBindTexture( GL_TEXTURE_2D, TextureIDs[6] );	// NeptuneTex must have already been created when this is called
	// 	glPushMatrix( );
...
Keyboard( unsigned char c, int x, int y )
{
	...
	switch( c )
	{
		...
		case 't':
			// 0 No texture - lit sphere with good scale
			// 1 Proper scale with texture image GL_REPLACE
			// 2 Proper scale with the texture image GL_modulated
			TextureSetting = (TextureSetting + 1) % 3; // for step 7, value between 0 - 2
			break;
