...
float NowS0, NowT0, NowD; // for the square
...
Display( )
{
	...
	// _____ END SQUARE CODE ____

	// BEGIN ELIPSE CODE
	if (KeytimePattern) { // check if keytime is on
		NowSc = keySc.GetValue( nowTime );
		NowTc = keyTc.GetValue( nowTime );
		Pattern.SetUniformVariable( "uSc", NowSc );
		Pattern.SetUniformVariable( "uTc", NowTc );
	}

	if (TimePattern) { // check if Time pattern is on
		float A = 0.09;  // Amplitude
		float B = 3.;    // Frequency (adjust as needed)
		float D = 0.1;  // Vertical shift

		NowRs = A * sin(B * F_PI * Time) + D;
		NowRt = 2 * (A * sin(B * F_PI * Time) + D);
		//printf("sin(Time): %f Time: %f \n", sin(F_PI * Time), Time);
		//printf("NowRs: %f	NowRt: %f Time: %f\n", NowRs, NowRt, Time);
	...
	// Pattern.SetUniformVariable( "uRs" , .1f  );
	// Pattern.SetUniformVariable( "uRt" , .1f );
	// END ELIPSE CODE
...
InitGraphics( )
{
	...
	// keytime stuff, time for Sc and Tc
	keySc.Init( );
	keySc.AddTimeValue(  0.0,  0.500 );
	keySc.AddTimeValue(  2.5,  0.2 );
	keySc.AddTimeValue(  5.,  0.6 );
	keySc.AddTimeValue(  7.5,  0.2 );
	keySc.AddTimeValue(  10.0,  .5 );

	keyTc.Init( );
	keyTc.AddTimeValue(  0.0,  0.500 );
	keyTc.AddTimeValue(  2.5,  0.1 );
	keyTc.AddTimeValue(  5.,  .2 );
	keyTc.AddTimeValue(  7.5,  .1 );
	keyTc.AddTimeValue(  10.0,  0.5 );
