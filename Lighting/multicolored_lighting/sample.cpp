...
Display( )
{
	...
	if (PointOrSpot == 0) {
		SetPointLight( GL_LIGHT0, 1., 1., 0., GRed, GGreen, GBlue );
	} else {
		SetSpotLight( GL_LIGHT0, 1., 1., 0., 0., -1., 0., GRed, GGreen, GBlue );
	}
	...
	glTranslatef(1., 0., 0.);
	glScalef(.4, .4, .4);
	SetMaterial( 1.f, 0.f, 0.f, 2.f );
	glCallList( CatList ); // put a cat in the scene
	glPopMatrix();

	// Spheres to test lighting
	// DULL Sphere - red
	// glPushMatrix();
	// glTranslatef(1., .5, 0.);
	// SetMaterial( 1.f, 0.f, 0.f, 20.f );
	// glCallList( SphereList ); // put a sphere in the scene
	// glPopMatrix();

	// INBETWEEN sphere
	glPushMatrix();
	glTranslatef(-.5, .5, 0.87);
	SetMaterial( 0.f, 1.f, 0.f, 64.f );
	glCallList( SphereList ); // put a sphere in the scene
	glPopMatrix();
