...
int SphereDL, VenusDL, EarthDL, MarsDL, JupiterDL, SaturnDL, UranusDL, NeptuneDL;
//int VenusTex, EarthTex, MarsTex, JupiterTex, SaturnTex, UranusTex, NeptuneTex;
int TextureIDs[7] = {
	0, // venus
	1, // earth
	2,
...
// Define an array to store planet names
char fileNames[][20] = {
    "venus.bmp",
    "earth.bmp",
    "mars.bmp",
...
for (int i = 0; i < 7; ++i) {
    ...
    if (texture != NULL) {
        //fprintf(stderr, "Opened '%s': width = %d ; height = %d\n", file, width, height);
		// 
        // Generate a new texture ID for each texture
        glGenTextures(1, (GLuint *) &TextureIDs[i]);
...
InitLists( )
{
	...
	glNewList( VenusDL, GL_COMPILE );
		glBindTexture( GL_TEXTURE_2D, TextureIDs[0] );	// venus Tex must have already been created when this is called
		glPushMatrix( );
